...
#Setting up the multiprocess processing part
    ...
    num_processes = multiprocessing.cpu_count() - free_cores - num_priority_tasks 
    ...
    for i in range(num_priority_tasks):

        process_name = 'P_priority{}'.format(str(i))

        new_process = multiprocessing.Process(target=processing_message, args=(logging_queue, worker_configurer,
        process_name, priority_tasks, speaker_email_data, sting, watermark,slug, sheet_id, sheet_name))
    ...
    while True:
        ...
        for message in messages:
            ...
            try:
                ...
                if priority == 1:
                    priority_tasks.put([priority_tasks])  
                else:
                    tasks.put([message])
