    ...
    while True:
        ...
        if message == 0:
            ...
            try:
                ...
                if isinstance(data['TITLE'].values[0],str):
                    title = data['TITLE'].values[0] 
                else:
                    title = ''

                description = 'Web Summit Master class'
            ...
            try:
                with open(f'/home/ubuntu/Talkbot/Facebook_video/output_data/data_{message}.csv','a') as f:
...
def main(input_bucket, output_bucket,free_cores= 0, priority_cores = 1):
    ...
    """
    lookup_data = pd.read_csv('./WS20_data/WS20_masterclasses.csv')
    logger = logging.getLogger(__name__)
    ...
    while True:
        ...
        for message in messages:
            ...
            try:
                temp = lookup_data[lookup_data['Omnia Code'] == uuid]
                if len(temp) == 1:
                    data = temp 
