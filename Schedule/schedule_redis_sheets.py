...
def get_redis():
    ...
    df = pd.DataFrame(out)
    df = df.transpose()
    df = df.reindex(columns=['title','description','location','speakers','speakers_for_emails','start_time','end_time','id','uuid','block','priority','set_private','resend_emails','status','vimeo_link','youtube_link','s3_link_public','s3_link_raw']) 
